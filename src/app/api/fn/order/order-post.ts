/* tslint:disable */
/* eslint-disable */
import { HttpClient, HttpContext, HttpResponse } from '@angular/common/http';
import { Observable } from 'rxjs';
import { filter, map } from 'rxjs/operators';
import { StrictHttpResponse } from '../../strict-http-response';
import { RequestBuilder } from '../../request-builder';

import { CashInPaymentKeyResponse } from '../../models/cash-in-payment-key-response';
import { CreateOrderRequest } from '../../models/create-order-request';

export interface OrderPost$Params {
      body?: CreateOrderRequest
}

export function orderPost(http: HttpClient, rootUrl: string, params?: OrderPost$Params, context?: HttpContext): Observable<StrictHttpResponse<CashInPaymentKeyResponse>> {
  const rb = new RequestBuilder(rootUrl, orderPost.PATH, 'post');
  if (params) {
    rb.body(params.body, 'application/*+json');
  }

  return http.request(
    rb.build({ responseType: 'json', accept: 'text/json', context })
  ).pipe(
    filter((r: any): r is HttpResponse<any> => r instanceof HttpResponse),
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<CashInPaymentKeyResponse>;
    })
  );
}

orderPost.PATH = '/Order';
